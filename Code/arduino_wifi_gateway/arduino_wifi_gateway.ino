...
unsigned int getChecksum() {
  ...
  for (int i=0; i<incoming.length(); i++) {
    //msgSum += incoming.charAt(i)*(i+1);
    msgSum += incoming.charAt(i);
  }
  Serial.println(msgSum);
...
void loop() {
  ...
  if (flag) {
    unsigned int check_sum = getChecksum();
    byte byte1 = check_sum / 256;
    byte byte2 = check_sum % 256;
    Serial.println(incoming);
    Serial.println("*** Recieved ***");
    Serial.println(sigByte1);
    Serial.println(sigByte2);
    Serial.println("*** Calculating ***");
    Serial.println(byte1);
    Serial.println(byte2);
    
    // check for communication's errors //
    if (byte1 == sigByte1 && byte2 == sigByte2) { 
